...
<Steps>
    ...
    <li>... and maybe a bit of envious of its features</li>
    <li>This meeting is about some features in rust that I personally think very cool</li>
    ...
    <li>Shameless Self Advertisement</li>
    <li>And a full disclaimer</li>
</Steps>

---

# I am envious of Cargo 
# ( Their package manager )

---

## I am envious of Cargo ( Their package manager )
...
# I'm envious of built in code generation 
# (Derive)
...
//# deriveInterface! ( json_serialize, json_deserialize )
struct Person
{
    std::string name;
    uint8_t age;
...
import sys
import shutil
import json_interface_generator

def generate_file(input_file, output_file):
    shutil.copyfile(input_file, output_file)
    json_interface_generator.process_file(output_file)
...
struct Person : public json_serializable, public json_deserializable
{
    std::string name;
    uint8_t age;
    std::vector<std::string> phones;
    ...
    void from_json(const json& data)
    {
        ...
        age = data["age"].get<uint8_t>();
        phones = data["phones"].get<std::vector<std::string>>();
    }
...
# I'm envious of linear typesystem of Rust
...
<Steps>
    <li>Of course rust is a very big language, so we just scratched the surface here </li>
    <li>There are many interersting concepts in rust especially about the memory managment and multithreading </li>
    <li>Concepts of lifetimes is certainly a important point </li>
    <li>But as of today.. </li>
    <li>We saw how can we imporve build systems to achieve a package manager</li>
    <li>Maybe we want to create some stuff from our code</li>
    <li>Or we want single use for all of our data structures</li>
